...
def test_single_quarter():
    """測試單一季度的爬取"""
    logger.info("=" * 60)
    logger.info("測試 Selenium MOPS 爬蟲 - 單一季度")
    logger.info("=" * 60)

    test_year = 112  # 民國 112 年
    test_season = 3  # 第 3 季

    with MOPSSeleniumScraper(headless=False) as scraper:  # 顯示瀏覽器視窗以便觀察
        logger.info(f"\n[測試] 爬取 {test_year}Q{test_season} 所有上市公司資料")

        result = scraper.fetch_balance_sheet('', test_year, test_season)
...
def test_multiple_quarters():
    """測試多個季度的爬取"""
    logger.info("=" * 60)
    logger.info("測試 Selenium MOPS 爬蟲 - 多個季度")
    logger.info("=" * 60)

    with MOPSSeleniumScraper(headless=True) as scraper:  # 無頭模式，加快速度
        logger.info("\n爬取民國 112 年所有季度...")

        all_data = scraper.fetch_all_quarters(start_year=112)
